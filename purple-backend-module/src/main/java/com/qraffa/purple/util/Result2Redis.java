...
import com.qraffa.purple.util.judge.JudgeResult;
...
public class Result2Redis {
    ...
    static Logger logger = LoggerFactory.getLogger(Result2Redis.class);
    ...
    public static void parseResult(String uid, List<RunResult> resultList) {
        ...
        for (RunResult run : resultList) {
            ...
            logger.info("{} signal:{} code:{} result:{} error:{}",
                    sb.toString(), run.getJudgeResult().getSignal(),
                    run.getJudgeResult().getExit_code(), run.getJudgeResult().getResult(),
                    run.getJudgeResult().getError());
